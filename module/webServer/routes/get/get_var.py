...
async def get_data_by_var_id(request:Request):
    try:
        ...
        if not var_id:
            ...
            vars = await MySqlConn.rawSqlCmd(f'''SELECT var_name, simple_code, var_type, var_permission from device_variables WHERE id = "{var_id}" ORDER BY id ASC''')
...
async def get_vars_by_device_id(request:Request):
    try:
        ...
        if not device_id:
            ...
            vars = await MySqlConn.rawSqlCmd(f'''SELECT id, var_name, simple_code, var_type, var_permission, latest_value, updated_at, var_full_code 
